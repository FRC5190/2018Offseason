...
public class TimingUtil {
    ...
    public static <S extends State<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(
        ...
        for (int i = 0; i < states.size(); ++i) {
            ...
            while (true) {
                ...
                dupl(reverse, (List<TimingConstraint<S>>) constraints, (ConstrainedState<S>) constraint_state);
    ...
    private static <S extends State<S>> void dupl(boolean reverse, List<TimingConstraint<S>> constraints, ConstrainedState<S> constraint_state) {
        for (final TimingConstraint<S> constraint : constraints) {

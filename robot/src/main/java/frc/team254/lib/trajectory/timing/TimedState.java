...
public class TimedState<S extends State<S>> implements State<TimedState<S>> {
    ...
    public TimedState<S> interpolate(TimedState<S> other, double x) {
        final double new_t = Interpolable.Companion.interpolate(t(), other.t(), x);
        final double delta_t = new_t - t();
        if (delta_t < 0.0) {
            return other.interpolate(this, 1.0 - x);
        }
        boolean reversing = velocity() < 0.0 || (Math.abs(velocity()) < 1E-9 && acceleration() < 0.0);
    ...
    public boolean equals(final Object other) {
        ...
        return state().equals(ts.state()) && Math.abs(t() - ts.t()) < 1E-9;
    }
}
